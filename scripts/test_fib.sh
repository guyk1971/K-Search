...
# --- NVIDIA Inference API backend (uncomment to use) ---
# BACKEND="inference"
# MODEL_NAME="gemini-3-pro"    # short names auto-resolved to provider paths
# # Set INFERENCE_API_KEY in .env or environment
...
if [[ "${BACKEND}" != "inference" ]]; then
  BACKEND_ARGS+=(--api-key "${API_KEY}" --base-url "${BASE_URL}")
fi
